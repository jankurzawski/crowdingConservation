...
for roi = 1 : 4
    ...
    subplot(1,2,1)
    h(roi)=errorbar(1:length(c_boot_mean),c_boot_mean,c_boot_ci(:,1),c_boot_ci(:,2),'color',nanmean(mycmap{roi}),'LineWidth',2);
    xticks(1:20)
    xticklabels(leg)
    hold on
    
    if roi == 4
        s =plot(xlim,[1.36 1.36],'--k')
        ylabel('c [mm]')
        xtickangle(90)
        xlabel('Eccentricity bins')
        set(gca,'Fontsize',20)
    end
    
    subplot(1,2,2)
    errorbar(1:length(r2_boot_mean),r2_boot_mean,r2_boot_ci(:,1),r2_boot_ci(:,2),'color',nanmean(mycmap{roi}),'LineWidth',2);
    xticks(1:20)
    xticklabels(leg)
    hold on
    
    if roi == 4
        s =plot(xlim,[0.43 0.43],'--')
        xtickangle(90)
        ylabel('R^2')
        xlabel('Eccentricity bins')
        set(gca,'Fontsize',20)
...
legend(cat(2,h,s),{'V1';'V2';'V3';'hV4';'Our estimate for hV4 (fullmap)'},'box','off')
